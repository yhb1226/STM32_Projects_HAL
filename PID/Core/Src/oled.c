...
void OLED_W_SCL(uint8_t val)
{
    ...
    HAL_GPIO_WritePin(SCL_GPIO_Port, SCL_Pin, (GPIO_PinState)val);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/

}

void OLED_W_SDA(uint8_t val)
{
	/*根据BitValue的值，将SDA置高电平或者低电平*/
	HAL_GPIO_WritePin(SDA_GPIO_Port, SDA_Pin, (GPIO_PinState)val);
	/*如果单片机速度过快，可在此添加适量延时，以避免超出I2C通信的最大速度*/
...
{
    // 延时（也可用 HAL_Delay(1)，但此处保留原循环亦无妨）
    uint32_t i, j;
    for (i = 0; i < 1000; i++)
        for (j = 0; j < 1000; j++);
